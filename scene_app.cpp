...
void SceneApp::InitPlayer()
{
	...
	player_body_def.position = b2Vec2(-2.0f, 4.0f);
...
void SceneApp::GameUpdate(float frame_time)
{
	...
	b2Vec2 verticalSpead(0.0f, 400.0f);
	b2Vec2 horizontalSpead(5.0f * xaxisval, 0.0f);
	if (controller->buttons_pressed() & gef_SONY_CTRL_CROSS)
	{
		player_body_->ApplyForceToCenter(verticalSpead, true);
	}

	if (xaxisval == 1)
	{
		player_body_->ApplyForceToCenter(horizontalSpead, true);
	}
	else if (xaxisval == -1)
	{
		player_body_->ApplyForceToCenter(horizontalSpead, true);
...
void SceneApp::GameRender()
{
	...
	float cameraPosX = player_body_->GetPosition().x + 4.5f;
	// view
	gef::Vector4 camera_eye(cameraPosX, 2.0f, 9.0f);
	gef::Vector4 camera_lookat(cameraPosX, 2.0f, 0.0f);
	gef::Vector4 camera_up(0.0f, 1.0f, 0.0f);
	gef::Matrix44 view_matrix;
	view_matrix.LookAt(camera_eye, camera_lookat, camera_up);
	renderer_3d_->set_view_matrix(view_matrix);

	//Temporary UI score/Life text
	// draw 3d geometry
	renderer_3d_->Begin();
	...
	DrawHUD();
	font_->RenderText(sprite_renderer_, gef::Vector4(100.0f, 510.0f, -0.9f), 1.0f, 0xffffffff, gef::TJ_LEFT, "Pos X: %.1f", player_body_->GetPosition().x);
	font_->RenderText(sprite_renderer_, gef::Vector4(100.0f, 480.0f, -0.9f), 1.0f, 0xffffffff, gef::TJ_LEFT, "Pos y: %.1f", player_body_->GetPosition().y);
	sprite_renderer_->End();
}
